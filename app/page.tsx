...
export default function Scorecard() {
  ...
  return (
    <div className="container">
      <header>
        <p className="eyebrow">Agent Guardian · Free 3-minute diagnostic</p>
        <h1>Your agents are not expensive. Your leaks are.</h1>
        <p className="lead">Answer 15 questions. See which pipe is open — visibility, caps, routing, agent loops, or habits — and get the first fix before you buy another tool.</p>
      </header>

      <main>
        {stage === 'land' && (
          <section>
            <button className="btn" onClick={() => setStage('quiz')}>Find my leak</button>
            <p className="micro">No credit card. About three minutes. You get a score and a next step.</p>
            <div className="card">
              <ul className="bullets">
                <li>Built for people running agents, not enterprise FinOps teams</li>
                <li>Names the leak. Does not lecture you about "AI strategy"</li>
                <li>Pairs with Agent Guardian when you are ready to put a hard stop on spend</li>
              </ul>
            </div>
          </section>
        )}
        ...
        {stage === 'quiz' && (
          <section>
            <p className="eyebrow">Question {currentQ + 1} of 15 · {QUESTIONS[currentQ][0]}</p>
            <div className="q">
              <p>{QUESTIONS[currentQ][1]}</p>
              <div className="scale">
                {[1, 2, 3, 4, 5].map(n => (
                  <label key={n}>
                    <input
                      ...
                      name="ans"
                      value={n}
                      checked={answers[currentQ] === n}
                      onChange={() => handleAnswer(n)}
                    />
                    {n}
                  </label>
                ))}
            ...
            <button className="btn" onClick={handleNextQuestion}>
              {currentQ === 14 ? 'See results gate' : 'Next'}
            </button>
          </section>
        )}
        ...
        {stage === 'gate' && (
          <section>
            <h2>Your score is ready. Where should we send it?</h2>
            <p>Enter the email you actually check. We show the category breakdown and the first fix here, and we can send a copy.</p>
            <label className="field" htmlFor="name">First name (optional)</label>
            <input
              id="name"
              type="text"
              value={name}
            ...
            />
            <p></p>
            <button className="btn" onClick={handleShowResults} disabled={loading}>
              {loading ? 'Loading...' : 'Show my leak'}
            </button>
            <p className="micro">By continuing you agree we can email your results and occasional product updates for Agent Guardian. Unsubscribe anytime.</p>
          </section>
        )}
